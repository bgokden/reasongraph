...
class _SayYes:
    """A span linker that ties exactly one (effect, cause) pair, whatever the embeddings say,
    and records the order it was asked in."""
    def __init__(self, pair): self.pair = pair; self.asked = []
    def predict(self, pairs):
        self.asked += list(pairs)
        return [5.0 if (a, b) == self.pair else -8.0 for a, b in pairs]
...
async def test_span_linker_ties_paraphrased_hops_cosine_misses():
    ...
    # the matcher is asked (effect of the earlier hop, cause of the later hop), never reversed
    pair = ("the greenhouse night temperature drops sharply", "A sharp drop in greenhouse night temperature")
    for linker, expect in ((None, None), (_SayYes(pair), 2)):
        ...
        try:
            ...
            if expect is None:
                ...
                assert all(a != b for a, b in linker.asked)
